...
from app.services.scheme_engine import match_schemes_async, get_all_schemes
...
async def list_schemes(
    ...
    - Live database schemes from PostgreSQL (Central & State schemes)
    """
    cost = project_cost
    ...
    if any([user_capital, primary_skill, sector_interest, district, state]):
        user_profile = UserProfile(
            ...
            location={"district": district or "Ahmedabad", "state": state or "Gujarat"}
        )

    return await match_schemes_async(db=db, user_profile=user_profile, project_cost=cost, business_category=category)


@router.get("/by-opportunity/{opportunity_id}", response_model=List[SchemeMatchItem])
async def get_schemes_for_opportunity(opportunity_id: str, db: AsyncSession = Depends(get_db)):
    """
    ...
    return await match_schemes_async(db=db, user_profile=None, project_cost=cost, business_category=opportunity_id)


@router.post("/matches", response_model=List[SchemeMatchItem])
async def get_scheme_matches(request: SchemeMatchRequest, db: AsyncSession = Depends(get_db)):
    """
    Match the entrepreneur/business profile to potentially relevant official schemes.
    """
    try:
        matches = await match_schemes_async(
            db=db,
